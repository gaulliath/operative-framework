...
impl Node {
    pub async fn on_help(&self) -> Result<(), ErrorKind> {
        ...
        for (name, description, usage) in opf_models::command::LISTS {
            let fields = vec![name.to_string(), description.to_string(), usage.to_string()];
            rows.push(fields);
        }
        let _ = send_event_to(
            &self.self_tx,
            (Domain::CLI, Event::ResponseTable((headers, rows))),
        )
        .await;

        // listing of target type
        let headers = vec!["target_type available".to_string()];
        let mut rows = vec![];

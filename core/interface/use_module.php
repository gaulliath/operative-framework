...
if(!isset($_POST['running_module']) || !isset($_POST['module_name']))
{
    die("No module");
}
unset($_POST['running_module']);
$_POST['exec'] = 'use_module';
$string_post = json_encode($_POST);
?>
<!DOCTYPE html>
<html>

<head>
    <title>Operative framework - web interface</title>
    <script
    ...
    <style>
        ...
        .opt
        {
            ...
            font-size: 10px;
            border: 0;
            border-bottom: 1px solid #906f6f;
            margin-left: 7px;
            color: #527a8a;
        }
        #title_option
...
<body>
<div id='logo'></div>
<div id="information_module">result of <span id="module_name"></span> | <a href="index.php">back</a> </div>
<div id="load_module">
</div>
</body>
<script>
    $(document).ready(function () {
        $("#load_module").html('loading...')
        var post_data = <?= $string_post; ?>;
        $.post(api_url,post_data, function(data){
            $('#module_name').html(post_data['module_name']);
            $("#load_module").html('');
            console.log(data)
            if(typeof data.data == 'string')
            {
                var tpl = '<div class="report_view">';
                tpl = tpl + '<p class="element"><span class="information">information:</span> '+data.data+'</p>'
                tpl = tpl + '</div>';
                $('#load_module').append(tpl)
            }
                ...
                $.each(data.data, function(index, value){
                    ...
                    if(typeof value == 'object')
                    {
                        $.each(value, function(index, value){
                            tpl = tpl + '<p class="element"><span class="information">'+index+':</span>'+value+'</p>'
                        })
                    }
                    else
                    {
                       tpl = tpl + '<p class="element">'+value+'</p>'
                    }
                    tpl = tpl + '</div>';
                    $('#load_module').append(tpl)
                });

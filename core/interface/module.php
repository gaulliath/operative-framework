...
<body>
<div id='logo'></div>
<div id="information_module"><a href="index.php">back</a></div>
<div id="module_option">
    <p id="title_option">Configure module <b><?= strip_tags($_GET['use']); ?></b></p>
    <form action="use_module.php" method="post">
        <input type="hidden" name="module_name" value="<?= strip_tags($_GET['use']); ?>"/>
        <div id="field_list">
        </div>
        <div id="start_module"><input type="submit" name="running_module" id="running_module" value="Run"></input></div>
    </form>
</div>
</body>
<script>
    $(document).ready(function () {
        call_api('requirement_module',{'module_name':'<?= strip_tags($_GET['use']) ?>'}, function(result)
        {
            if(result == false)
            {
                $('#field_list').html('Module not found')
            }
            else
            {
